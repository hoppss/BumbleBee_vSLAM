...
{
	/*performs the most basic matching across frames with a simple
	 * Lowe rejection scheme set by a threshold,
	 * each incoming stereoKP is compared to the most recent descriptor 
	 * seen in each SalientPoint, the score is computed.
	 * the two lowest scores are computed and its ratio computed,
	 * anything that is higher by some threshold than this base "best" ratio is considered 
	 * to be a poor match and is then added as a new distinctive SalientPoint,
	 * else it is added into the SalientPoint*/
...
void SlidingWindow::addNewKP(StereoKP KP,int frameNumber)
{
	...
	features_[overall_feat_]->correspondences_.push_back(KP);
